...
// POST /api/users/:id/moods  { local_date, mood }
...
export const createUserDailyMood = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { local_date, mood } = req.body as { local_date?: string; mood?: string };
  ...
  const allowed = ['happy','calm','focused','relaxed'];
  if (!mood || !allowed.includes(mood)) {
    return res.status(400).json({ success: false, message: `mood must be one of ${allowed.join(', ')}` });
  }
  ...
  const record = await DailyMood.create({ user_id: userId, local_date, mood } as any);
